...
export const CRITICAL_DATABASE_INDEX_REQUIREMENTS = Object.freeze([
  ...
  requirement('notification-outbox.delivery-queue', NotificationOutbox, { status: 1, nextAttemptAt: 1, createdAt: 1 }),
]);
...
const optionMismatches = (actual = {}, expected = {}) => {
  ...
  if (
    expected.partialFilterExpression === true
    && (!actual.partialFilterExpression || typeof actual.partialFilterExpression !== 'object')
  ) {
    mismatches.push('partialFilterExpression is missing');
  }
...
const evaluateIndexes = ({ requirements, getIndexes }) => {
  const missing = [];
  const mismatched = [];
  const checked = [];
...
export const buildCriticalIndexDefinitionReport = () => evaluateIndexes({
...
const evaluateLiveIndexes = ({ requirements, indexesByCollection }) => evaluateIndexes({
  requirements,
...
const applySafeIndexRepairs = async ({ requirements, report }) => {
  ...
  for (const mismatch of report.mismatched) {
    ...
    if (!item || !canSafelyConvertToTtl(item, checkedEntry)) {
      continue;
    }
