...
describe('password reset security', () => {
  ...
  it('counts concurrent invalid reset attempts atomically', async () => {
    const { user } = await signupWithAgent({ firstName: 'Reset', lastName: 'ConcurrentAttempts' });
    const { agent, csrfToken, code } = await requestResetCode(user.email);

    const responses = await Promise.all(
      Array.from({ length: 6 }, () => postAuth(agent, csrfToken, 'verify-reset-code', {
        email: user.email,
        code: '000000',
      }))
    );
  ...
  it('allows only one concurrent password reset to consume a code', async () => {
    const { user } = await signupWithAgent({ firstName: 'Reset', lastName: 'ConcurrentConsume' });
    const { agent, csrfToken, code } = await requestResetCode(user.email);

    const responses = await Promise.all([
      postAuth(agent, csrfToken, 'reset-password', {
        email: user.email,
        code,
  ...
  it('keeps one active reset record during concurrent forgot-password requests', async () => {
    const { user } = await signupWithAgent({ firstName: 'Reset', lastName: 'ConcurrentIssue' });
    const first = await resetRequestAgent();
    const second = await resetRequestAgent();

    await Promise.all([
      postAuth(first.agent, first.csrfToken, 'forgot-password', { email: user.email }),
      postAuth(second.agent, second.csrfToken, 'forgot-password', { email: user.email }),
    ]);

    await expect(PasswordReset.countDocuments({ email: user.email })).resolves.toBe(1);
  });
