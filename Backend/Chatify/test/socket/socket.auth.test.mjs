...
describe('authenticated Socket.IO handshake', () => {
  ...
  it('rejects a sessionless legacy access token during the handshake', async () => {
    const server = await startServer();
    const user = await createUser({ firstName: 'Legacy', lastName: 'Socket' });
    ...
    const socket = trackSocket(connectSocketWithCookie(server.url, `accessToken=${token}`));
    const errorPromise = waitForSocketEvent(socket, 'connect_error');
  ...
  it('disconnects an established socket when its session is revoked', async () => {
    const server = await startServer();
    const signup = await signupWithAgent({ firstName: 'Live', lastName: 'Revocation' });
    const connected = await connectSocketWithReady(server.url, extractCookieHeader(signup.response));
    const socket = trackSocket(connected.socket);
    const revokedPromise = waitForSocketEvent(socket, 'auth:revoked', 3000);
    const disconnectedPromise = waitForSocketEvent(socket, 'disconnect', 3000);
    const csrfToken = await getCsrfForAgent(signup.agent);
    ...
    await expect(revokedPromise).resolves.toMatchObject({ reason: 'sessions_revoked' });
    await expect(disconnectedPromise).resolves.toBeTruthy();
    expect(socket.connected).toBe(false);
  });

  it('shares socket event limits across reconnects for the same session', async () => {
    const server = await startServer();
    const signup = await signupWithAgent({ firstName: 'Rate', lastName: 'Reconnect' });
    const cookieHeader = extractCookieHeader(signup.response);
    const first = await connectSocketWithReady(server.url, cookieHeader);
    trackSocket(first.socket);
    const nonexistentChatId = '64b000000000000000000001';

    for (let attempt = 0; attempt < 20; attempt += 1) {
      const response = await emitWithAck(first.socket, 'chat:join', nonexistentChatId);
      expect(response.code).not.toBe('rate_limited');
    }

    expect(await emitWithAck(first.socket, 'chat:join', nonexistentChatId)).toMatchObject({
      ok: false,
      code: 'rate_limited',
    });
    first.socket.disconnect();

    const second = await connectSocketWithReady(server.url, cookieHeader);
    trackSocket(second.socket);

    expect(await emitWithAck(second.socket, 'chat:join', nonexistentChatId)).toMatchObject({
      ok: false,
      code: 'rate_limited',
    });
  });
